...
int main (int argc, char **argv)
{
    ...
    if ((buffer_count_sem = sem_open (SEM_BUFFER_COUNT_NAME, O_CREAT, 0660, 3)) == SEM_FAILED) {
        perror ("sem_open"); exit (1);
    }
...
void *producer (void *arg)
{
    ...
    for (i = 0; i < 5; i++) {
        printf ("Producer-Thread%d: waiting at {buffer_count_sem}\n", my_id);
...
/*There might be multiple producers. We must ensure that
  ...
  Aquire mutex semaphore*/
        if (sem_wait (mutex_sem) == -1) {
            perror ("sem_wait: mutex_sem"); exit (1);
        }
        printf ("Producer-Thread%d: Entered in CriticalSection\n", my_id);

        j = buffer_index;
/*Pushing string into buffer*/
        sprintf (buf [j], "BuffData%d", ++count);
        printf("Producer-Thread%d:Pushed String at buf[%d]: %s\n", my_id, j, buf[j]);
/*Updating Index*/
        buffer_index++;
 ...
 /*Release mutex semaphore*/
        ...
        printf ("Producer-Thread:%d: Exit from CriticalSection\n", my_id);
...
/*Tell spooler that there is a string to print: V (spool_signal_sem);*/
        ...
        printf ("ThreadId:%d signaled Spool-Thread\n");
...
/*Is there a string to print? P (spool_signal_sem);*/
        printf ("\nSpooler-Thread: Waiting {spool_signal_sem}\n");
        if (sem_wait (spool_signal_sem) == -1) {
            perror ("sem_wait: spool_signal_sem"); exit (1);
        }
        printf ("Spooler-Thread: Received signal from ProducerThread\n");

        printf ("Spooler-Thread buf[%d]: %s\n", buffer_print_index, buf[buffer_print_index]);
...
/* Contents of one buffer has been printed.
   ...
   Release buffer: V (buffer_count_sem);  */
        ...
        printf ("Spooler-Thread signaled to {sem_wait(buffer_count_sem)}\n");
    }
}
